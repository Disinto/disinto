<!-- last-reviewed: df4b9c234f71d972ebb2c2ce8dc85ec71e81afdf -->
...
| `nomad/jobs/edge.hcl` | submitted via `lib/init/nomad/deploy.sh` | Caddy reverse proxy; routes /forge, /woodpecker, /staging, /chat; uses `disinto/edge:local` image built by `bin/disinto --with edge`; the vault-action dispatcher (`docker/edge/dispatcher.sh`) runs as a background process inside the Caddy container via `entrypoint-edge.sh` (the separate `dispatcher` task was removed in #1184 — it ran the agents entrypoint with all roles, never `dispatcher.sh`); **the Caddy task uses `network_mode = "host"`** — upstreams are `127.0.0.1:<port>` (forgejo :3000, woodpecker :8000, chat :8080), not Docker hostnames (#1031, #1034); `FORGE_URL` rendered via Nomad service discovery template (`nomadService "forgejo"` — switched from Consul `service` lookup to Nomad native service discovery, #1114) to handle bridge vs. host network differences (#1034); `/forge/*` handler adds `uri strip_prefix /forge` before proxying to forgejo (#1103); `/staging/*` strips `/staging` prefix before proxying (#1079); WebSocket endpoint `/chat/ws` uses `header_up` inside `reverse_proxy` block (moved from handle-block top level — Caddy rejects top-level `header_up`, #1117); `/chat/ws` added for streaming (#1026); mobile browsers get `Cache-Control: no-cache` on index.html to avoid stale voice-client.js (#860); engagement measurement endpoint `/api/engagement` proxies to local `docker/edge/engagement-server.py` (#975) |
...
| `nomad/jobs/agents-dev-qwen.hcl` | submitted via `lib/init/nomad/deploy.sh` | Per-role agent job running the `dev` role against the local Qwen model; one role per job keeps the llama slot count a property of the deployment (S4 split, #1123, #1125) |
| `nomad/jobs/agents-review-qwen.hcl` | submitted via `lib/init/nomad/deploy.sh` | Per-role agent job running the `review` role against the local Qwen model; one role per job keeps the llama slot count a property of the deployment (S4 split, #1123, #1125) |
| `nomad/jobs/agents-gardener-qwen.hcl` | submitted via `lib/init/nomad/deploy.sh` | Per-role agent job running the `gardener` role against the local Qwen model; one role per job keeps the llama slot count a property of the deployment (S4 split, #1123, #1125) |
...
2. **`nomad job validate nomad/jobs/*.hcl`** (loop, one call per file)
   ...
   step does NOT catch:
     - cross-file references (`source = "forgejo-data"` typo against the
       `host_volume` list in `nomad/client.hcl`) — that's a scheduling-time
