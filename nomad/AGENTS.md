<!-- last-reviewed: 0a9df661b68829d6230f201b1f19fbf6444a1088 -->
...
| `nomad/acl-policies/chat-ops.hcl` | installed via `lib/init/nomad/chat-init.sh` | Nomad ACL policy for the chat-Claude factory control surface (#650); grants `submit-job`/`read-job`/`read-logs` in default namespace, `read` on agents and nodes; token stored in Vault at `kv/disinto/chat` key `nomad_token` |
| `nomad/jobs/agents-supervisor-opus.hcl` | submitted via `lib/init/nomad/deploy.sh` | Supervisor agent running exclusively on Claude Opus via OAuth (issue #589); replaces the supervisor loop previously in the edge Caddy task; read-write `docker.sock` access for container management |
| `nomad/jobs/vault-runner.hcl` | submitted via `lib/init/nomad/deploy.sh` | Parameterized batch job for Vault action dispatch (S5.3, #990); replaces `docker run --rm vault-runner-${action_id}` pattern; dispatched by edge dispatcher via `nomad job dispatch` |
| `nomad/jobs/edge-threads-gc.hcl` | submitted via `lib/init/nomad/deploy.sh` | Periodic batch job for threads-state GC (runs daily 03:00 UTC); deletes completed/failed/error threads older than THREADS_TTL (default 7 days) via `bin/threads.sh` |
