# =============================================================================
# nomad/jobs/agents-gardener-qwen.hcl — gardener-role agent job (local Qwen
# model)
#
...
#   per-slot cap. This job draws from the same pool as the dev and review
#   jobs, so it carries the same 100k lane (50% override) as they do, not
#   a wider one.
#
#   CLAUDE_MAX_TURNS stays 60 deliberately: more turns at a thrashing lane
#   buys more thrash. Widening the lane is the lever.
#
# Host_volume contract: agent-data-gardener, project-repos, ops-repo, and
# factory-projects are declared in nomad/client.hcl and created by
# lib/init/nomad/cluster-up.sh. The gardener gets a dedicated
# agent-data-gardener source (not the shared "agent-data" source the dev
# and review jobs use) so its state cannot collide with theirs (#1123).
# =============================================================================

job "agents-gardener-qwen" {
  type        = "service"
  datacenters = ["dc1"]
  ...
  group "agents" {
    ...
    # agent-data: dedicated per-role host_volume source (#1123) — the dev
    # and review qwen jobs share the base "agent-data" source, and the
    # gardener's logs/journal/step state must not collide with theirs.
    # Declared in nomad/client.hcl as agent-data-gardener.
    volume "agent-data" {
      type      = "host"
      source    = "agent-data-gardener"
      read_only = false
    }
    ...
    task "agents" {
      ...
      env {
        ...
        # step logs over the Woodpecker REST API so the agent can see why a
        # PR's CI failed. The server is addressed by container IP because
...
EOT
      ...
      # nomad/jobs/agents.hcl. This job authenticates as the gardener bot:
      # FORGE_TOKEN/FORGE_PASS come from kv/disinto/bots/gardener.
      #
