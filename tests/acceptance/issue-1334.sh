...
# formula by project kind (#1314 removed; the kind key itself is gone with
# #1338). The planner always loads formulas/run-planner.toml — the research
# formula formulas/run-planner-research.toml is deleted, and the old switch
# test tests/acceptance/issue-1314.sh is gone (the runner only runs AC for
# open issues; leaving it would lie).
#
...
# 4. Behaviour: planner_formula_file() returns the run-planner.toml path —
...
ac_log "checking planner_formula_file() returns the run-planner.toml path"
