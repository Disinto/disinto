...
# longer selects the formula by project kind. Every kind branch in
# architect/architect-run.sh is deleted — the software branch is the only
# path: architect_formula_file() returns formulas/run-architect.toml, the
# tracking green gate is closed + deployed + acceptance rc=0, and the
# role/pitch text is sprint-shaped. The research formula
...
#      string and no kind env-var string, and is still syntactically
#      valid (bash -n).
...
# 1. No research-formula or kind env-var references in architect-run.sh;
#    still parses. (The env var name is built from a split literal so this
#    test file itself stays free of the retired string.)
PK="PROJECT_""KIND"
ac_log "checking architect-run.sh has no run-architect-research or kind env-var string"
! grep -q "run-architect-research" "$ARCHITECT_RUN" \
  || ac_fail "architect/architect-run.sh still references run-architect-research"
! grep -q "$PK" "$ARCHITECT_RUN" \
  || ac_fail "architect/architect-run.sh still references the kind env var"
...
# 4. Behaviour: architect_formula_file() returns the run-architect.toml
#    path — the kind switch is gone (#1338). ac_pick_in_subshell runs the
#    extracted function in a throwaway subshell with a fixed fake
#    FACTORY_ROOT (shared helper — tests/lib/acceptance-helpers.sh).
ac_log "checking architect_formula_file() returns the run-architect.toml path"
