...
# ── 1. Wiring: dev-poll sources the tape lib and calls the emitters ────────
...
# isolated subshell; capture the subshell's output and exit status.
run_proposal() {
...
# extracted #1399 emitter in an isolated subshell (stub curl on PATH, real
...
# ── 2. Fresh pick: the sibling started epoch file is written next to the ────
# ── id file, and holds an integer unix epoch ────────────────────────────────
TAPE1="$TMP_DIR/tape-happy"
rc=0
out="$(run_proposal "$TAPE1" 1452)" || rc=$?
...
NOW_BEFORE="$(date -u +%s)"
rc=0
out2="$(run_outcome "$TAPE1" 1452 42 1 1)" || rc=$?
...
rc=0
out4="$(run_outcome "$TAPE3" 9999 44 1 1)" || rc=$?
...
TAPE4="$TMP_DIR/tape-future-start"
...
rc=0
out5="$(run_outcome "$TAPE4" 9996 45 1 1)" || rc=$?
...
TAPE5="$TMP_DIR/tape-repick"
rc=0
out6="$(run_proposal "$TAPE5" 9997)" || rc=$?
ac_assert_eq "$rc" "0" "first pick must return 0 (got $rc): $out6"
STARTED_FILE_9997="/tmp/dev-proposal-started-${PROJECT_NAME}-9997"
FIRST_STARTED="$(cat "$STARTED_FILE_9997")"
rc=0
out7="$(run_proposal "$TAPE5" 9997)" || rc=$?
ac_assert_repick "$rc" "$out7"
ac_assert_eq "$(wc -l < "$TAPE5/tape.jsonl")" "1" \
  "two calls on one issue must leave exactly one tape line (re-pick guard)"
SECOND_STARTED="$(cat "$STARTED_FILE_9997")"
