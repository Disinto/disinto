...
# Operator-managed per-env factory project TOML directory (#794).
# Distinct from /srv/disinto/project-repos/_factory/projects/ (baked image
# path inside the agents container) — this host directory is the SOURCE of
# truth for per-box config and is mounted RO into agent jobs as the
# `factory-projects` host_volume.
FACTORY_PROJECTS_DIR="/srv/disinto/projects"
FACTORY_PROJECTS_DEFAULT_TOML="${FACTORY_PROJECTS_DIR}/disinto.toml"
...
[dry-run] Step 4b/9: seed default factory project TOML if empty (#794)
  → install -d -m 0775 -o ubuntu -g ubuntu /srv/disinto/projects
  → write /srv/disinto/projects/disinto.toml (only if dir empty)
...
# ── Step 4b: factory-projects dir + default TOML seed (#794) ─────────────────
# /srv/disinto/projects/ is the operator-managed source of truth for per-env
# project config (replaces the legacy /opt/disinto/projects/ location). It is
# mounted RO into agent jobs via the `factory-projects` host_volume. Seed a
# default disinto.toml when empty so a clean rebuild does not require manual
# operator intervention before agents come up.
#
# Ownership ubuntu:ubuntu (mode 0775) matches the conventional non-root user
# on the factory dev box, so the operator can edit TOMLs without sudo. Falls
# back to root:root if the ubuntu account does not exist.
if id ubuntu >/dev/null 2>&1; then
  install -d -m 0775 -o ubuntu -g ubuntu "$FACTORY_PROJECTS_DIR"
  chown ubuntu:ubuntu "$FACTORY_PROJECTS_DIR"
  chmod 0775 "$FACTORY_PROJECTS_DIR"
else
  log "user 'ubuntu' not found — leaving ${FACTORY_PROJECTS_DIR} as root:root"
fi

# Seed a default disinto.toml if /srv/disinto/projects/ contains no TOMLs at
# all — handles a clean nomad-box rebuild where no operator-managed config has
# been migrated in yet. The default points at the local Forgejo so agents can
# at least bootstrap and self-heal once the cluster is up. Operators replace
# it post-init with their real config.
if ! compgen -G "${FACTORY_PROJECTS_DIR}/*.toml" >/dev/null 2>&1; then
  log "seeding default factory project: ${FACTORY_PROJECTS_DEFAULT_TOML}"
  cat > "$FACTORY_PROJECTS_DEFAULT_TOML" <<'TOML'
# Auto-generated by lib/init/nomad/cluster-up.sh (#794)
...
repo_root       = "/home/agent/repos/disinto"
...
TOML
  if id ubuntu >/dev/null 2>&1; then
    chown ubuntu:ubuntu "$FACTORY_PROJECTS_DEFAULT_TOML"
  fi
  chmod 0644 "$FACTORY_PROJECTS_DEFAULT_TOML"
else
  log "factory-projects: existing TOML(s) present — not seeding default"
fi
