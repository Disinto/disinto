...
configure_git_creds() {
  ...
  if [ -z "$bot_user" ]; then
    # Forgejo may simply be booting (expired here in ~15s of retries is
    ...
    # crash-looping the container (#1238).
    local http_code api_base
    api_base="${FORGE_API_BASE:-${FORGE_URL}/api/v1}"
    http_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 \
      -H "Authorization: token ${FORGE_TOKEN}" "${api_base}/user" 2>/dev/null || echo "000")
