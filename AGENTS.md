<!-- last-reviewed: 1948b165df8066d38ceee04cd555d5ddca34cae8 -->
...
| AD-002 | **Concurrency is bounded per LLM backend, not per project.** One concurrent Claude session per OAuth credential pool; llama-server concurrency bounded by the shared KV pool (see (b)). | **(a) Anthropic OAuth** — each container uses per-session `CLAUDE_CONFIG_DIR`; native lockfile handles contention. (Rollback: `CLAUDE_EXTERNAL_LOCK=1`.) **(b) llama-server** — finite VRAM, one KV pool. Without `--kv-unified` the pool is per-slot, so one concurrent session per instance (parallel inference thrashes the cache → OOM) — llama agents serialize. With `--kv-unified` (disinto's deployment, #1069) the pool is shared and the budget is the **sum of all concurrent sessions'** context against `--ctx-size`; size each agent's autocompact lane (docs/agents-llama.md) so the sum leaves headroom. **(c) Disjoint backends parallelize freely.** `disinto-agents` (Anthropic) runs concurrently with `disinto-agents-llama` (llama). **(d) Per-project safety** enforced by `issue_claim` + per-issue worktrees (separate from this AD). |
