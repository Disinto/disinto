...
│                  pending-actions.json(l) — queued gardener actions (JSONL format)
...
├── docker/        Dockerfiles: reproduce, triage, runner; edge/ (Caddy + chat + voice + dispatcher + chat-skills: ack-inbox, check-inbox); voice/ (bridge.py, UI)
